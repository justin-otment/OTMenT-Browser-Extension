...
// ============================================
// === Fibonacci-based Adaptive Sleep System
// ============================================
let fibState = { a: 1, b: 1, step: 0 };

function nextFibonacci() {
  const next = fibState.a + fibState.b;
  fibState.a = fibState.b;
  fibState.b = next;
  fibState.step++;
  return fibState.a;
}

async function fibonacciSleep(base = 200, jitter = 0.1) {
  const fibFactor = nextFibonacci();
  const delay = base * fibFactor;
  const randomized = delay * (1 + (Math.random() * jitter - jitter / 2));
  console.log(`[OTMenT] 🕐 Fibonacci sleep: ${randomized.toFixed(0)} ms (factor ${fibFactor})`);
  return new Promise(res => setTimeout(res, randomized));
}

function resetFibonacci() {
  fibState = { a: 1, b: 1, step: 0 };
}
