...
async function getTargetURLs(cfg, resumeRow = null, processedUrls = new Set()) {
  ...
  // --- Fetch reference columns (F:L) for same row window
  ...
  // --- Fetch Site column (B) for same row window
  ...
  // --- Fetch log columns (U:AE) for same rows
  ...
  // --- Build absolute row entries, preserving blanks
  let results = [];
  for (let i = 0; i < urlVals.length; i++) {
    ...
    results.push({
      ...
      row,
      refs: refVals[i] || [],
      siteVal: siteVals[i]?.[0] || "" // preload site value
    });
  }
...
chrome.runtime.onStartup.addListener(() => {
  console.log("[OTMenT] Chrome startup detected — launching runNavigator()");
  runNavigator();
});

chrome.runtime.onInstalled.addListener(() => {
  console.log("[OTMenT] Extension installed — launching runNavigator()");
  runNavigator();
});
